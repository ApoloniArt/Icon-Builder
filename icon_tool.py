...
============================================
WHAT IT DOES:
  1. Converts any PNG images into proper Windows .ico files (all sizes)
...
DLL_PATH    = "icon_library.dll"  # your icon store (created automatically)
...
def cmd_build():
    ...
    if not pngs:
        print(f"No PNG files found in '{os.path.abspath(PNG_FOLDER)}'.")
        print("Drop your ComfyUI exports there and run this again.")
        return

    print(f"\nConverting {len(pngs)} PNG(s) → .ico ...")
    ico_files = []
    for p in pngs:
...
def cmd_clearcache():
    """Delete Windows icon cache so updated icons display immediately."""
    import subprocess
    ...
    print("  ✓ Done — Explorer restarted with fresh icon cache.")
    print("    Icons should now display correctly on your desktop.")
